...
rm "build/libs/MelloLib-v$version.jar"
rm "build/libs/MelloLib-v$version-sources.jar"
rm "build/libs/MelloLib-v$version-javadoc.jar"

copy_jar() {
    echo "processing archive '$base_name'"
    local jar="mellolib-${base_name}-v${version}.jar"
    local sources="mellolib-${base_name}-v${version}-sources.jar"
    local javadoc="mellolib{base_name}-v${version}-javadoc.jar"

    local src="mellolib-${base_name}/build/libs"
